"""
Interactive Nine-Box model (percentual por quadrante + descrições corrigidas)

Corrigi o problema: as chaves do dicionário de descrições estavam com a ordem invertida. Agora as descrições estão indexadas como (Potencial, Performance) — ou seja, (Categoria_Y, Categoria_X) — exatamente como na imagem que você enviou.
...
def main(page: ft.Page):
    ...
    def atualiza_dados(e=None):
        ...
        grafico64, msgerro = gera_ninebox(pilar=pilar, competencia=competencia, participante=participante)
        if grafico64 == None:
        ...
        grafico_pilar64, msgerro = gera_gráfico_pilar(pilar=pilar, competencia=competencia, participante=participante)
        ...
        grafico_comp64, msgerro = gera_gráfico_Competencia(pilar=pilar, competencia=competencia, participante=participante)
        if grafico_comp64 == None:
            img_Comp.visible = False
        else:
            img_Comp.visible = True

        grafico_compar64, msgerro_compar = gera_gráfico_Comparativo(pilar=pilar, competencia=competencia, participante=participante)
    ...
    page.title = "Avaliação de Desempenho"
    page.scroll = "auto"
    ...
    btn_limpar_campos = ft.ElevatedButton("Limpar", on_click=limpar_campos, width=100 )
